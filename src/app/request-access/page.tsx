...
export default function RequestAccessPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    setError(null);
    if (!name || !email || !requestedRole) { setError("Name, email, and role are required"); return; }
    setSubmitting(true);
    try {
  ...
  if (submitted) {
    return (
      <section className="mx-auto flex min-h-screen max-w-sm flex-col justify-center px-6 py-24 text-center">
        ...
        <Link href="/login" className="mt-6 text-sm font-medium text-blue-700 hover:text-blue-800">Back to login</Link>
      </section>
    );
  }
  ...
  return (
    <section className="mx-auto flex min-h-screen max-w-sm flex-col justify-center px-6 py-24">
      ...
      <p className="mt-2 text-sm text-slate-500">A manager will review and create your account.</p>

      <Card className="mt-8 rounded-2xl border-slate-200/70 bg-white/80 p-6 shadow-[0_8px_30px_-12px_rgba(37,99,235,0.15)] backdrop-blur-xl">
      ...
      <p className="mt-6 text-center text-sm text-slate-500">
        ...
        <Link href="/login" className="font-medium text-blue-700 hover:text-blue-800">Log in</Link>
      </p>
    </section>
  );
}
