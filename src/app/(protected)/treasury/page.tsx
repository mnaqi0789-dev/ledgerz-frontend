    ...
    <main className="mx-auto min-h-[calc(100vh-4rem)] max-w-7xl px-4 py-8 sm:px-6">
      ...
      <Card className="mt-7 gap-0 rounded-md border border-slate-200 py-0 shadow-none">
        <div className="overflow-x-auto">
          <Table>
            <TableHeader><TableRow><TableHead>Asset</TableHead>{isManager && <><TableHead>Quantity</TableHead><TableHead>Cost basis</TableHead><TableHead>Current price</TableHead></>}<TableHead>Current value</TableHead><TableHead>Gain / loss</TableHead></TableRow></TableHeader>
            <TableBody>
              {isLoading && <TableRow><TableCell colSpan={isManager ? 6 : 3} className="h-28 text-center text-slate-500">Loading treasury...</TableCell></TableRow>}
              {isError && <TableRow><TableCell colSpan={isManager ? 6 : 3} className="h-28 text-center text-rose-600">Treasury could not be loaded.</TableCell></TableRow>}
              {!isLoading && !isError && holdings?.length === 0 && <TableRow><TableCell colSpan={isManager ? 6 : 3} className="h-28 text-center text-slate-500">No treasury holdings recorded.</TableCell></TableRow>}
              {holdings?.map((holding) => {
                const full = holding as TreasuryHoldingFull;
                return <TableRow key={holding.assetName}><TableCell className="font-medium capitalize text-slate-900">{holding.assetName}</TableCell>{isManager && <><TableCell>{full.quantity}</TableCell><TableCell>{formatCurrency(full.buyPrice)}</TableCell><TableCell>{formatCurrency(full.currentPrice)}</TableCell></>}<TableCell className="font-medium">{formatCurrency(holding.currentValue)}</TableCell><TableCell className={holding.gainLossPercent >= 0 ? "text-emerald-700" : "text-rose-700"}>{formatPercent(holding.gainLossPercent)}</TableCell></TableRow>;
              })}
            </TableBody>
          </Table>
        </div>
